...
const Post = ({ post }) => {
	...
	const {mutate:deletePost, isPending:isDeleting } = useMutation({ //Delete Post mutation
		mutationFn: async () => {
			try {
				const res = await fetch(`/api/post/${post._id}` , { //fetch the post to delete
					method: "DELETE",
				});
				const data = await res.json(); //turn the post into json
		...
		onSuccess: () => {
			...
			queryClient.invalidateQueries({ queryKey: ["posts"] }); //Delete post from UI
		}
	});

	const {mutate:likePost, isPending:isLiking} = useMutation({ //Like Post mutation
		mutationFn: async () => {
			try {
				const res = await fetch(`/api/post/like/${post._id}`, { //Fetch post to be liked
					method: "POST",
				});
				const data = await res.json(); //Turn post into json format
				if (!res.ok) {
